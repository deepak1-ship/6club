...
const TAB_SECTION_MAP = {
  ...
  function countDownTimer({ GAME_TYPE_ID }) {
    const getTimeMSS = (countDownDate) => {
      ...
      let minute = 0;
      if(GAME_TYPE_ID == 3) minute = Math.ceil(minutes % 1);
      else if(GAME_TYPE_ID == 5) minute = Math.ceil(minutes % 3);
      else if(GAME_TYPE_ID == 10) minute = Math.ceil(minutes % 5);
      else minute = 0; // 30s game

      let seconds1 = 0; 
      let seconds2 = 0;

      if(GAME_TYPE_ID == 1) {
         // 30 Seconds Game Logic
         let msInMinute = distance % 60000;
         // We split the minute into two 30s blocks.
         // If msInMinute > 30000, we are in the first block (display 30..0)
         // If msInMinute <= 30000, we are in the second block (display 30..0)
         // Actually, just mod 30000 gives 0-29999.
         let msIn30s = msInMinute % 30000;
         let totalSeconds = Math.floor(msIn30s / 1000);
         // Prevent 60 or 30 showing when exactly 0
         if (totalSeconds < 0) totalSeconds = 0;
         
         seconds1 = Math.floor(totalSeconds / 10);
         seconds2 = totalSeconds % 10;
      } else {
         seconds1 = Math.floor((distance % (1000 * 60)) / 10000);
         seconds2 = Math.floor(((distance % (1000 * 60)) / 1000) % 10);
      }
    ...
    countDownInterval1 = setInterval(function () {
      ...
      // Display minute only if not 30s game
      if (GAME_TYPE_ID !== "1") {
...
const displayResultHandler = ({ status, amount, period, result }) => {
  ...
  $("#lottery_results_box").html(resultHtml);
    ...
    let timeStr = GAME_TYPE_ID == 1 ? "30 seconds" : GAME_TYPE_ID == 3 ? "1 minute" : GAME_TYPE_ID == 5 ? "3 minutes" : "5 minutes";
  $("#popup_game_details").html(`Period:K3 ${timeStr} ${period}`);
