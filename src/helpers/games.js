...
// Get nth 30-second interval since day start (for 30-second games)
export function getNth30SecondSinceDayStart() {
  const now = moment();
  const hours = now.hours();
  const minutes = now.minutes();
  const seconds = now.seconds();
  // Each hour has 120 thirty-second intervals, each minute has 2
  return (hours * 120) + (minutes * 2) + (seconds >= 30 ? 2 : 1);
}

export function generatePeriod(gameRepresentationId, interval = 1) {
  const TodayPeriod = moment().format("YYYYMMDD");
  const nthMinuteSinceDayStart = getNthMinuteSinceDayStart();

  // Calculate period number based on interval (e.g. for 5min: 0-4 -> 1, 5-9 -> 2)
  const periodNumber = Math.floor(nthMinuteSinceDayStart / interval) + 1;

  const paddedPeriodNumber = _.padStart(
    periodNumber.toString(),
    4,
    "0",
  );

  let NewGamePeriod = `${TodayPeriod}${gameRepresentationId}${paddedPeriodNumber}`;

  return parseInt(NewGamePeriod);
}

// Generate period for 30-second games
export function generate30sPeriod(gameRepresentationId) {
  const TodayPeriod = moment().format("YYYYMMDD");
  const nth30SecSinceDayStart = getNth30SecondSinceDayStart();
  const paddedNth30Sec = _.padStart(
    nth30SecSinceDayStart.toString(),
    4,
    "0",
  );

  let NewGamePeriod = `${TodayPeriod}${gameRepresentationId}${paddedNth30Sec}`;
