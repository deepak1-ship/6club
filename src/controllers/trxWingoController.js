...
import { generatePeriod, generate30sPeriod } from "../helpers/games.js";

let _blockCache = { data: null, fetchedAt: 0 };
const BLOCK_CACHE_TTL_MS = 2000;

const fetchTronBlocks = async (retries = 3) => {
  const now = Date.now();
  if (_blockCache.data && now - _blockCache.fetchedAt < BLOCK_CACHE_TTL_MS) {
    return _blockCache.data;
  }
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      _blockCache = { data: response.data, fetchedAt: Date.now() };
  ...
  try {
    ...
    if (isPendingGame) {
      ...
      if (isAdminManipulatedResult) {
        ...
        const blockData = await fetchTronBlocks();

        const NextBlock = blockData.data
          .map((item) => {
            return {
              id: item.number,
              hash: item.hash,
    ...
    // Map game IDs to new intervals
    if (game == 1) {
      NewGamePeriod = generate30sPeriod(gameRepresentationId);
    } else if (game == 3) {
      NewGamePeriod = generatePeriod(gameRepresentationId, 1);
    } else if (game == 5) {
      NewGamePeriod = generatePeriod(gameRepresentationId, 3);
    } else if (game == 10) {
      NewGamePeriod = generatePeriod(gameRepresentationId, 5);
    } else {
      NewGamePeriod = generatePeriod(gameRepresentationId, game);
    }
