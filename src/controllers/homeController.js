...
const getActivityBanners = async (req, res) => {
  try {
...
const getCustomBanners = async (req, res) => {
  try {
...
const getLangCurrency = async (req, res) => {
  try {
    const data = await settingsModel.langCurrency.getAll();
...
const getInviteTasks = async (req, res) => {
  try {
    const tasks = await settingsModel.inviteTasks.getAll();

    // Map database fields to frontend expected keys
    const formattedTasks = tasks.map(task => ({
      taskID: task.id,
      taskAmount: task.task_amount,
      taskPeople: task.task_people,
      rechargeAmount: task.recharge_amount,
      taskRechargePeople: task.task_people,
      // These would need to be calculated per-user from their invite/recharge data
      efficientPeople: 0,
      rechargePeople: 0,
      isFinshed: false,
      isReceive: 0,
      beginDate: task.created_at || null,
      endDate: null
    }));

    return res.json({ status: true, data: { taskList: formattedTasks } });
  } catch (error) {
    return res.json({ status: false, message: error.message });
  }
};
