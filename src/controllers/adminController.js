...
const ctvProfilePage = async (req, res) => {
  ...
  return res.render("manage/ctv/profile.ejs", { phone });
};
...
const membersPage = async (req, res) => {
  return res.render("manage/members/list.ejs");
};

const subordinatePage = async (req, res) => {
  return res.render("manage/members/subordinate.ejs");
};

const ctvPage = async (req, res) => {
  return res.render("manage/ctv/list.ejs");
};
...
const statistical = async (req, res) => {
  return res.render("manage/ctv/create.ejs");
};
...
const levelSetting = async (req, res) => {
  return res.render("manage/ctv/levels.ejs");
};

const CreatedSalaryRecord = async (req, res) => {
  return res.render("manage/ctv/salary.ejs");
};

const DailySalaryEligibility = async (req, res) => {
  return res.render("manage/ctv/eligibility.ejs");
};
...
const totalJoin = async (req, res) => {
  ...
  if (rows.length > 0) {
    ...
    const [winGo10] = await connection.query(
...
const updateBank = async (req, res) => {
  try {
    const { phone, bank_id, name_user, stk, name_bank, chi_nhanh } = req.body;

    // Validate required fields
    if (!phone || !name_user || !stk || !name_bank || !chi_nhanh) {
      return res.status(200).json({
    ...
    if (existingBank.length === 0) {
      // Create new bank record if doesn't exist
      await connection.query(
        "INSERT INTO user_bank (phone, name_user, stk, name_bank, chi_nhanh, tp, time) VALUES (?, ?, ?, ?, ?, 'BANK_CARD', ?)",
        [phone, name_user, stk, name_bank, chi_nhanh, timeNow],
      );
    } else {
      // Update existing bank record
      await connection.query(
        "UPDATE user_bank SET name_user = ?, stk = ?, name_bank = ?, chi_nhanh = ? WHERE phone = ?",
        [name_user, stk, name_bank, chi_nhanh, phone],
      );
    }
...
const settingBank = async (req, res) => {
  try {
    ...
    if (typer == "momo") {
      ...
      const qrCodeImage = req.body.qr_code_image || usdtWalletAddress; // Use uploaded QR or fallback to USDT address

      await connection.query(
        "INSERT INTO bank_recharge SET name_bank = ?, name_user = ?, stk = ?, qr_code_image = ?, type = 'momo'",
        [bankName, username, upiId, qrCodeImage],
      );
...
const newupi = async (req, res) => {
  return res.render("manage/newfinance/upi.ejs");
};

const newusdt = async (req, res) => {
  return res.render("manage/newfinance/usdt.ejs");
};

const newusdtrate = async (req, res) => {
  return res.render("manage/newfinance/usdtrate.ejs");
};

const newdepositupdate = async (req, res) => {
  return res.render("manage/newfinance/depositupdate.ejs");
};

const newwithdrawl = async (req, res) => {
  return res.render("manage/newfinance/withdrawal.ejs");
};

const newwidthdrawalaccepted = async (req, res) => {
  return res.render("manage/newfinance/withdrawalaccepted.ejs");
};

const newwithdrawalrejected = async (req, res) => {
  return res.render("manage/newfinance/withdrawalrejected.ejs");
};

const newupdategetway = async (req, res) => {
  return res.render("manage/newfinance/updategateway.ejs");
};

const newpaymentgetway = async (req, res) => {
  return res.render("manage/newfinance/paymentgateway.ejs");
};

const newwithdrawlrules = async (req, res) => {
  return res.render("manage/newfinance/withdrawalrules.ejs");
};
...
const getPaymentGateways = async (req, res) => {
  try {
    const [rows] = await connection.execute(
      "SELECT * FROM new_payment_gateways ORDER BY id ASC LIMIT 10",
    );
    return res.status(200).json({
      message: "Success",
      status: true,
...
const savePaymentGateway = async (req, res) => {
  try {
    const {
      id,
      pay_name,
    ...
    if (id && id !== 'null') {
      ...
      await connection.execute(
        `UPDATE new_payment_gateways 
         SET pay_name = ?, min_price = ?, max_price = ?, bonus_percent = ?, recharge_amounts = ?, gift_amounts = ?
         WHERE id = ?`,
        [
          pay_name,
          min_price,
      ...
      const [countResult] = await connection.execute(
        "SELECT COUNT(*) as count FROM new_payment_gateways",
      );
      if (countResult[0].count >= 10) {
        return res.status(200).json({
          message: "Maximum 10 gateways allowed",
          status: false,
          timeStamp: timeNow,
        });
      }
      ...
      await connection.execute(
        `INSERT INTO new_payment_gateways (pay_name, min_price, max_price, bonus_percent, recharge_amounts, gift_amounts) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [
          pay_name,
          min_price,
    ...
    return res.status(200).json({
      message: "Failed to save gateway",
      status: false,
      timeStamp: timeNow,
    });
  }
...
const deletePaymentGateway = async (req, res) => {
  try {
    ...
    await connection.execute("DELETE FROM new_payment_gateways WHERE id = ?", [
      id,
    ]);
    return res.status(200).json({
...
const updateGatewayStatus = async (req, res) => {
  try {
    ...
    for (const gateway of gateways) {
      await connection.execute(
        "UPDATE new_payment_gateways SET is_active = ?, gateway_type = ? WHERE id = ?",
        [gateway.is_active, gateway.gateway_type || "manual", gateway.id],
      );
    }
    ...
    return res.status(200).json({
      message: "Failed to update gateways",
      status: false,
      timeStamp: timeNow,
    });
  }
...
// ============================================================
// UPI Management API Functions
// ============================================================

const getUPIList = async (req, res) => {
  try {
    const [rows] = await connection.execute(
      "SELECT * FROM new_upi_ids ORDER BY id ASC",
    );
    const [activeRow] = await connection.execute(
      "SELECT id FROM new_upi_ids WHERE is_active = 1 LIMIT 1",
    );
    return res.status(200).json({
      message: "Success",
      status: true,
      data: rows,
      activeId: activeRow.length > 0 ? activeRow[0].id : null,
      timeStamp: timeNow,
    });
  } catch (error) {
    console.error("Error fetching UPI list:", error);
    return res.status(200).json({
      message: "Failed to fetch UPI list",
      status: false,
      timeStamp: timeNow,
    });
  }
...
const addUPI = async (req, res) => {
  try {
    ...
    await connection.execute("INSERT INTO new_upi_ids (upi_id) VALUES (?)", [
      upi_id,
    ]);
    return res.status(200).json({
...
const deleteUPI = async (req, res) => {
  try {
    ...
    await connection.execute("DELETE FROM new_upi_ids WHERE id = ?", [id]);
    return res.status(200).json({
...
const setActiveUPI = async (req, res) => {
  try {
    ...
    await connection.execute("UPDATE new_upi_ids SET is_active = 0");
    // Then activate selected
    await connection.execute(
      "UPDATE new_upi_ids SET is_active = 1 WHERE id = ?",
      [id],
    );
    return res.status(200).json({
      message: "Active UPI updated successfully",
      status: true,
      timeStamp: timeNow,
    });
  } catch (error) {
    console.error("Error setting active UPI:", error);
    return res.status(200).json({
      message: "Failed to set active UPI",
      status: false,
      timeStamp: timeNow,
    });
  }
...
const getUPIImages = async (req, res) => {
  try {
    const [rows] = await connection.execute(
      "SELECT * FROM new_upi_images ORDER BY id ASC",
    );
    const [activeRow] = await connection.execute(
      "SELECT id FROM new_upi_images WHERE is_active = 1 LIMIT 1",
    );
    return res.status(200).json({
      message: "Success",
      status: true,
    ...
    console.error("Error fetching UPI images:", error);
    return res.status(200).json({
...
const uploadUPIImage = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(200).json({
    ...
    const filepath = "/uploads/upi/" + filename;
    await connection.execute(
      "INSERT INTO new_upi_images (filename, filepath) VALUES (?, ?)",
      [filename, filepath],
    );
    return res.status(200).json({
...
const deleteUPIImage = async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) {
      return res.status(200).json({
    ...
    await connection.execute("DELETE FROM new_upi_images WHERE id = ?", [id]);
    return res.status(200).json({
...
const setActiveImage = async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) {
      return res.status(200).json({
    ...
    // First, deactivate all
    await connection.execute("UPDATE new_upi_images SET is_active = 0");
    // Then activate selected
    await connection.execute(
      "UPDATE new_upi_images SET is_active = 1 WHERE id = ?",
      [id],
    );
    return res.status(200).json({
    ...
    return res.status(200).json({
      message: "Failed to set active image",
      status: false,
      timeStamp: timeNow,
    });
  }
...
// ============================================================
// USDT Management API Functions
// ============================================================

const getUSDTList = async (req, res) => {
  try {
    const [rows] = await connection.execute(
      "SELECT * FROM new_usdt_addresses ORDER BY id ASC",
    );
    const [activeRow] = await connection.execute(
      "SELECT id FROM new_usdt_addresses WHERE is_active = 1 LIMIT 1",
    );
    return res.status(200).json({
      message: "Success",
      status: true,
    ...
    console.error("Error fetching USDT list:", error);
    return res.status(200).json({
      message: "Failed to fetch USDT list",
      status: false,
      timeStamp: timeNow,
    });
  }
...
const addUSDT = async (req, res) => {
  try {
    const { wallet_address, network_type } = req.body;
    if (!wallet_address) {
      return res.status(200).json({
    ...
    await connection.execute(
      "INSERT INTO new_usdt_addresses (wallet_address, network_type) VALUES (?, ?)",
      [wallet_address, network_type || "TRC20"],
    );
    return res.status(200).json({
    ...
    return res.status(200).json({
      message: "Failed to add USDT address",
      status: false,
      timeStamp: timeNow,
    });
  }
...
const deleteUSDT = async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) {
      return res.status(200).json({
    ...
    await connection.execute("DELETE FROM new_usdt_addresses WHERE id = ?", [
      id,
    ]);
    return res.status(200).json({
    ...
    console.error("Error deleting USDT:", error);
    return res.status(200).json({
      message: "Failed to delete USDT address",
      status: false,
      timeStamp: timeNow,
    });
  }
...
const setActiveUSDT = async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) {
      return res.status(200).json({
    ...
    await connection.execute("UPDATE new_usdt_addresses SET is_active = 0");
    await connection.execute(
      "UPDATE new_usdt_addresses SET is_active = 1 WHERE id = ?",
      [id],
    );
    return res.status(200).json({
      message: "Active USDT updated successfully",
      status: true,
      timeStamp: timeNow,
    });
  } catch (error) {
    console.error("Error setting active USDT:", error);
    return res.status(200).json({
      message: "Failed to set active USDT",
      status: false,
      timeStamp: timeNow,
    });
  }
...
const getUSDTImages = async (req, res) => {
  try {
    const [rows] = await connection.execute(
      "SELECT * FROM new_usdt_images ORDER BY id ASC",
    );
    const [activeRow] = await connection.execute(
      "SELECT id FROM new_usdt_images WHERE is_active = 1 LIMIT 1",
    );
    return res.status(200).json({
      message: "Success",
      status: true,
    ...
    console.error("Error fetching USDT images:", error);
    return res.status(200).json({
      message: "Failed to fetch images",
      status: false,
      timeStamp: timeNow,
...
const uploadUSDTImage = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(200).json({
        message: "No file uploaded",
    ...
    await connection.execute(
      "INSERT INTO new_usdt_images (filename, filepath) VALUES (?, ?)",
      [filename, filepath],
    );
    return res.status(200).json({
      message: "Image uploaded successfully",
    ...
    console.error("Error uploading USDT image:", error);
    return res.status(200).json({
      message: "Failed to upload image",
      status: false,
      timeStamp: timeNow,
...
const deleteUSDTImage = async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) {
      return res.status(200).json({
    ...
    await connection.execute("DELETE FROM new_usdt_images WHERE id = ?", [id]);
    return res.status(200).json({
      message: "Image deleted successfully",
      status: true,
      timeStamp: timeNow,
    ...
    console.error("Error deleting USDT image:", error);
    return res.status(200).json({
      message: "Failed to delete image",
      status: false,
      timeStamp: timeNow,
...
const setActiveUSDTImage = async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) {
      return res.status(200).json({
    ...
    await connection.execute("UPDATE new_usdt_images SET is_active = 0");
    await connection.execute(
      "UPDATE new_usdt_images SET is_active = 1 WHERE id = ?",
      [id],
    );
    return res.status(200).json({
      message: "Active image updated successfully",
    ...
    console.error("Error setting active USDT image:", error);
    return res.status(200).json({
      message: "Failed to set active image",
      status: false,
      timeStamp: timeNow,
...
const getUSDTRate = async (req, res) => {
  try {
    const [rows] = await connection.execute(
      "SELECT usdt_rate FROM admin_ac LIMIT 1"
    );
    const rate = rows[0]?.usdt_rate ?? 92;
    return res.status(200).json({ status: true, rate: parseFloat(rate), timeStamp: timeNow });
  } catch (error) {
    return res.status(200).json({ status: true, rate: 92, timeStamp: timeNow });
  }
};

const setUSDTRate = async (req, res) => {
  try {
    const { rate } = req.body;
    if (!rate || isNaN(rate) || parseFloat(rate) <= 0) {
      return res.status(200).json({ status: false, message: "Valid rate is required", timeStamp: timeNow });
    }
    await connection.execute("UPDATE admin_ac SET usdt_rate = ?", [parseFloat(rate)]);
    return res.status(200).json({ status: true, message: "Exchange rate updated successfully", timeStamp: timeNow });
  } catch (error) {
    console.error("Error updating USDT rate:", error);
    return res.status(200).json({
      status: false,
      message: "Failed. Run: ALTER TABLE admin_ac ADD COLUMN usdt_rate DECIMAL(10,2) DEFAULT 92.00;",
      timeStamp: timeNow,
    });
  }
};
...
const adminController = {
  ...
  newdepositupdate,
  newwithdrawl,
  newwidthdrawalaccepted,
  newwithdrawalrejected,
  newupdategetway,
  newpaymentgetway,
  newwithdrawlrules,
  // Payment Gateway API
