...
const getBankCardInfo = async (req, res) => {
  ...
  try {
    ...
    const account = await AccountDB.getAllUserBankCards({
      userPhoneNumber: user.phone,
    });
...
const addUPI = async (req, res) => {
  ...
  try {
    ...
    const newAccount = await AccountDB.createUserUPI({
      userPhoneNumber: user.phone,
      upiId,
      upiName,
    });
...
const getUPIInfo = async (req, res) => {
  ...
  try {
    ...
    const account = await AccountDB.getUserUPI({
      userPhoneNumber: user.phone,
    });

    return res.status(200).json({
      account,
      message: "Successfully fetched UPI",
      status: true,
      timeStamp: timeNow,
    });
  } catch (error) {
...
const createWithdrawalRequest = async (req, res) => {
  ...
  try {
    ...
    if (accountId) {
      const row = await AccountDB.getUserBankEntryById({ id: accountId, userPhoneNumber: user.phone });
      if (row) {
        if (WITHDRAWAL_METHODS_MAP.UPI === withdrawalMethod) {
          account = { isAvailable: true, id: row.id, upiId: row.sdt, upiName: row.name_user };
        } else if (WITHDRAWAL_METHODS_MAP.USDT_ADDRESS === withdrawalMethod) {
          account = { isAvailable: true, id: row.id, mainNetwork: row.name_bank, usdtAddress: row.stk };
        } else {
          account = { isAvailable: true, id: row.id, bankName: row.name_bank, recipientName: row.name_user, bankAccountNumber: row.stk, IFSC: row.chi_nhanh, phoneNumber: row.tinh, upiId: row.sdt };
        }
      }
    } else if (WITHDRAWAL_METHODS_MAP.BANK_CARD === withdrawalMethod) {
      account = await AccountDB.getUserBankCard({ userPhoneNumber: user.phone });
    } else if (WITHDRAWAL_METHODS_MAP.UPI === withdrawalMethod) {
      account = await AccountDB.getUserUPI({ userPhoneNumber: user.phone });
    } else {
      account = await AccountDB.getUserUSDTAddress({ userPhoneNumber: user.phone });
    }
...
const AccountDB = {
  ...
  async getAllUserBankCards({ userPhoneNumber }) {
    const type = WITHDRAWAL_METHODS_MAP.BANK_CARD;
    let [accounts] = await connection.query(
      "SELECT * FROM user_bank WHERE `phone` = ? AND `tp` = ? ORDER BY id DESC",
      [userPhoneNumber, type],
    );

    if (!accounts || accounts.length === 0) {
      return { isAvailable: false, accounts: [] };
    }
  ...
  async createUserBankCard({
    ...
    let [accounts] = await connection.query(
      "SELECT * FROM user_bank WHERE `phone` = ? AND `tp` = ? ORDER BY id DESC LIMIT 1",
      [userPhoneNumber, type],
    );
  ...
  async getAllUserUSDTAddresses({ userPhoneNumber }) {
    const type = WITHDRAWAL_METHODS_MAP.USDT_ADDRESS;
    let [accounts] = await connection.query(
      "SELECT * FROM user_bank WHERE `phone` = ? AND `tp` = ? ORDER BY id DESC",
      [userPhoneNumber, type],
    ...
    return {
      ...
      accounts: accounts.map((a) => ({
        id: a.id,
        userPhoneNumber: a.phone,
        mainNetwork: a.name_bank,
        usdtAddress: a.stk,
        addressAlias: a.sdt,
        type,
      })),
  ...
  async getUserBankEntryById({ id, userPhoneNumber }) {
    const [rows] = await connection.query(
      "SELECT * FROM user_bank WHERE `id` = ? AND `phone` = ?",
      [id, userPhoneNumber],
    );
    return rows?.[0] || null;
  },
  ...
  async getUserUPI({ userPhoneNumber }) {
    ...
    return {
      ...
      userPhoneNumber: account.phone,
      upiId: account.sdt,
      upiName: account.name_user,
      type,
    };
  },
  async getAllUserUPIs({ userPhoneNumber }) {
    const type = WITHDRAWAL_METHODS_MAP.UPI;
    let [accounts] = await connection.query(
      "SELECT * FROM user_bank WHERE `phone` = ? AND `tp` = ? ORDER BY id DESC",
      [userPhoneNumber, type],
  ...
  async createUserUPI({ userPhoneNumber, upiId, upiName }) {
    let time = new Date().getTime();
    const type = WITHDRAWAL_METHODS_MAP.UPI;

    await connection.query(
      `INSERT INTO user_bank SET phone = '${userPhoneNumber}', sdt = '${upiId}', name_user = '${upiName}', tp = '${type}', time = '${time}'`,
    );
  ...
  async updateUserUPI({ userPhoneNumber, upiId, upiName }) {
    let time = new Date().getTime();
    const type = WITHDRAWAL_METHODS_MAP.UPI;

    await connection.query(
      `UPDATE user_bank SET sdt = '${upiId}', name_user = '${upiName}', time = '${time}' WHERE phone = '${userPhoneNumber}' AND tp = '${type}'`,
    );
...
const withdrawDB = {
  async getWithdrawalById(id) {
    ...
    return {
      ...
      withdrawal: withdrawalList.map((item) => {
        ...
        } else if (item.tp === WITHDRAWAL_METHODS_MAP.UPI) {
          return {
            id: item.id,
            orderId: item.id_order,
            phoneNumber: item.phone,
  ...
  async createUPIWithdrawalRequest({
    userPhoneNumber,
    upiId,
    upiName,
    amount,
    ...
    await connection.query(
      `INSERT INTO withdraw SET id_order = '${getOrderId()}', phone = '${userPhoneNumber}', sdt = '${upiId}', name_user = '${upiName}', tp = '${type}', time = '${time}', today = '${getTodayString()}', money = '${amount}'`,
    );
  },
...
const getAllUPICards = async (req, res) => {
  let timeNow = Date.now();
  try {
    let auth = req.cookies.auth;
    if (!auth) return res.status(400).json({ message: "Auth is required!", status: false, timeStamp: timeNow });
    const user = await getUserDataByAuthToken(auth);
    const account = await AccountDB.getAllUserUPIs({ userPhoneNumber: user.phone });
    return res.status(200).json({ account, message: "Success", status: true, timeStamp: timeNow });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Something went wrong!", status: false, timeStamp: timeNow });
  }
};

const getAllUSDTCards = async (req, res) => {
  let timeNow = Date.now();
  try {
    let auth = req.cookies.auth;
    if (!auth) return res.status(400).json({ message: "Auth is required!", status: false, timeStamp: timeNow });
    const user = await getUserDataByAuthToken(auth);
    const account = await AccountDB.getAllUserUSDTAddresses({ userPhoneNumber: user.phone });
    return res.status(200).json({ account, message: "Success", status: true, timeStamp: timeNow });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Something went wrong!", status: false, timeStamp: timeNow });
